...
# startup programs
[ -z "$WAYLAND_DISPLAY" ] && ~/.pixi/envs/retrovim/bin/kanata_touchcursor # requires disabling sudo password otherwise it will ask for password after login
[ -z "$WAYLAND_DISPLAY" ] && exec start-hyprland -- --config $DOTFILES/hypr/hyprland.lua
